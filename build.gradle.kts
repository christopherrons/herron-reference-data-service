...
springBoot {
	mainClass.set("com.herron.exchange.referencedataservice.server.ReferencedataserviceApplication")
}
...
tasks.register<Tar>("buildAndPackage") {
	...
	from(layout.projectDirectory.dir("reference-data-service-deploy/src/main/java/com/herron/exchange/referencedataservice/deploy/scripts")) {
		exclude("**/*.md")
	}
...
tasks.register("deployToServer") {
	remotes {
		withGroovyBuilder {
			"create"("webServer") {
				setProperty("host", "reference-data-service-1")
	...
	doLast {
		ssh.run(delegateClosureOf<org.hidetake.groovy.ssh.core.RunHandler> {
			session(remotes, delegateClosureOf<org.hidetake.groovy.ssh.session.SessionHandler> {
				...
				execute("tar -xf /home/herron/deploy/reference-data-service-${version}.tar.gz --directory /home/herron/deploy/")
				execute("rm /home/herron/deploy/reference-data-service-${version}.tar.gz ")
